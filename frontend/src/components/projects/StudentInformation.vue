...
defineProps<{ students: ProjectStudent[] }>()
</script>

<template>
  <div class="rounded-lg border border-line bg-paper p-5">
    <h3 class="mb-4 text-sm font-semibold text-charcoal">Student Information</h3>
    <EmptyState v-if="!students.length" icon="user" title="No students assigned" />
    <ul v-else class="space-y-3">
      <li v-for="s in students" :key="s.student_id" class="flex items-center gap-3">
        <Avatar :label="s.full_name" size="md" />
        <div class="min-w-0">
          <p class="truncate text-sm font-medium text-charcoal">{{ s.full_name }}</p>
